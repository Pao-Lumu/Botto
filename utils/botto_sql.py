...
class AccountSQL(BaseSQL):
    def add(self, user_id, day=None, month=None, year=None):
        if not self.exists(user_id):
            self.cursor.execute(
                "INSERT INTO account VALUES(?,?,?,?)", (user_id, day, month, year,))
        else:
            self.cursor.execute(
                "UPDATE account SET bday_day=?, bday_month=?, bday_year=? WHERE user_id=?", (day, month, year, user_id))
        self.db.commit()
    ...
    def get_user_birthday(self, user_id):
        if self.exists(user_id):
            self.cursor.execute(
                "SELECT bday_day, bday_month, bday_year FROM account WHERE user_id=?", (user_id,))
    ...
    def get_users_with_birthday(self, day, month):
        self.cursor.execute(
            "SELECT user_id, bday_year FROM account WHERE bday_day=? AND bday_month=?", (day, month,))
...
class PastaSQL(BaseSQL):
    def add(self, args: tuple):
        self.cursor.execute(
            '''INSERT INTO pasta VALUES (?,?,?,?,?,?,?)''', args)
        self.db.commit()

    def exists(self, pasta_tag):
        self.cursor.execute(
            """SELECT * FROM pasta WHERE pasta_tag=?""", (pasta_tag,))
    ...
    def pasta_owned(self, pasta_tag, user_id):
        self.cursor.execute(
            """SELECT * FROM pasta WHERE pasta_tag=? AND creator_id=?""", (pasta_tag, user_id))
        if self.cursor.fetchone() is None:
            return False
        return True
    ...
    def get(self, pasta_tag):
        self.cursor.execute(
            """SELECT pasta_text,uses FROM pasta WHERE pasta_tag=?""", (pasta_tag,))
        return self.cursor.fetchone()

    def get_owned(self, user_id):
        self.cursor.execute(
            """SELECT pasta_text,uses FROM pasta WHERE creator_id=?""", (user_id,))
        return self.cursor.fetchall()
    ...
    def delete(self, pasta_tag):
        self.cursor.execute(
            """DELETE FROM pasta WHERE pasta_tag=?""", (pasta_tag,))
        self.db.commit()

    def update_text(self, pasta_tag, pasta_text):
        self.cursor.execute(
            '''UPDATE pasta SET pasta_text=? WHERE pasta_tag=?''', (pasta_text, pasta_tag))
        self.db.commit()

    def update_uses(self, uses, pasta_tag):
        self.cursor.execute(
            '''UPDATE pasta SET uses=? WHERE pasta_tag=?''', (uses, pasta_tag))
        self.db.commit()

    def popular(self):
        self.cursor.execute(
            '''SELECT pasta_tag,uses FROM pasta ORDER BY uses DESC LIMIT 12''')
        return self.cursor.fetchall()

    def top(self):
        self.cursor.execute(
            '''SELECT pasta_tag,net_vote FROM pasta ORDER BY net_vote DESC LIMIT 12''')
        return

    def vote_up(self, pasta_tag):
        self.cursor.execute(
            '''UPDATE pasta SET dislikes=likes+1, net_vote=net_vote+1''')
        return

    def vote_down(self, pasta_tag):
        self.cursor.execute(
            '''UPDATE pasta SET dislikes=dislikes+1, net_vote=net_vote-1''')
        return
